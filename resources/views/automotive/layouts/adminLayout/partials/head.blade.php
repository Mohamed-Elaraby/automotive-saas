...
@if (Route::is(['icon-feather', 'notes']))
    <!-- Feather CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/icons/feather/feather.css')}}">
@endif

    <!-- Tabler Icon CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/tabler-icons/tabler-icons.min.css')}}">

    <!-- Fontawesome CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/fontawesome/css/fontawesome.min.css') }}">
    <link rel="stylesheet" href="{{ url('theme/plugins/fontawesome/css/all.min.css') }}">

    <!-- Iconsax CSS -->
    <link rel="stylesheet" href="{{ url('theme/css/iconsax.css')}}">

    <!-- Simplebar CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/simplebar/simplebar.min.css')}}">

    <!-- Datatable CSS -->
    <link rel="stylesheet" href="{{ url('theme/css/dataTables.bootstrap5.min.css')}}">
...
@if (Route::is(['add-blog', 'admin-dashboard', 'clear-cache', 'cronjob', 'currencies', 'customer-css', 'customer-js', 'customer-dashboard', 'database-backup', 'edit-blog', 'email-reply', 'email-settings', 'email-templates', 'email', 'gdpr-cookies', 'localization-settings', 'maintenance-mode', 'preference-settings', 'prefixes-settings', 'seo-setup', 'sitemap', 'sms-gateways', 'storage', 'super-admin-dashboard', 'system-backup', 'system-update','blog-tags']))
    ...
    <link rel="stylesheet" href="{{ url('theme/plugins/bootstrap-tagsinput/bootstrap-tagsinput.css')}}">
@endif

@if (Route::is(['contact-messages', 'security-settings', 'suppliers', 'users']))
    <!-- intltelinput CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/intltelinput/css/intlTelInput.min.css')}}">
@endif
...
@if (Route::is(['add-blog', 'add-product', 'bank-account-settings', 'barcode-settings', 'custom-fields', 'edit-blog', 'edit-customer', 'edit-product', 'esignatures', 'file-manager', 'form-editors', 'invoice-settings', 'invoice-templates-settings', 'invoice-templates', 'maintenance-mode', 'payment-methods', 'sass-settings', 'seo-setup', 'tax-rates', 'thermal-printer', 'todo-list', 'todo']))
    <!-- Quill CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/quill/quill.snow.css')}}">
@endif

@if (Route::is(['gdpr-cookies']))
    <!-- Summernote CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/summernote/summernote-lite.min.css')}}">
@endif

@if (Route::is(['icon-bootstrap']))
    <!-- Bootstrap Icon CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/icons/bootstrap/bootstrap-icons.min.css')}}">
@endif

@if (Route::is(['icon-flag']))
    <!-- Flag CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/icons/flags/flags.css')}}">
@endif

@if (Route::is(['icon-ionic']))
    <!-- Ionic CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/icons/ionic/ionicons.css')}}">
@endif

@if (Route::is(['icon-material']))
    <!-- Material CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/material/materialdesignicons.css')}}">
@endif

@if (Route::is(['icon-pe7']))
    <!-- Pe7 CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/icons/pe7/pe-icon-7.css')}}">
@endif

@if (Route::is(['icon-remix']))
    <!-- Remix Icon CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/icons/remix/remixicon.css')}}">
@endif

@if (Route::is(['icon-simpleline']))
    <!-- Simpleline CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/simpleline/simple-line-icons.css')}}">
@endif

@if (Route::is(['icon-themify']))
    <!-- Themify CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/icons/themify/themify.css')}}">
@endif

@if (Route::is(['icon-typicon']))
    <!-- Typicon CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/icons/typicons/typicons.css')}}">
@endif

@if (Route::is(['icon-weather']))
    <!-- Weather CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/icons/weather/weathericons.css')}}">
@endif
...
@if (Route::is(['account-settings', 'add-credit-notes', 'add-debit-notes', 'add-delivery-challan', 'add-invoice', 'add-purchases-orders', 'add-purchases', 'add-quotation', 'calendar', 'companies', 'customer-account-settings', 'customer-add-quotation', 'customer-details', 'customer-invoice-details', 'customer-invoices', 'customer-plans-settings', 'customer-security-settings', 'customers', 'edit-credit-notes', 'edit-debit-notes', 'edit-delivery-challan', 'edit-invoice', 'edit-purchases-orders', 'edit-purchases', 'edit-quotation', 'expenses', 'incomes', 'index', 'invoice', 'layout-default', 'layout-dark', 'layout-mini', 'layout-rtl', 'layout-single', 'layout-transparent', 'layout-without-header', 'membership-addons', 'notes', 'payments', 'plans-billing', 'profile', 'security-settings', 'social-feed', 'super-admin-dashboard', 'supplier-payments', 'suppliers', 'tickets-list', 'todo-list', 'todo']))
	<!-- Datetimepicker CSS -->
	<link rel="stylesheet" href="{{ url('theme/css/bootstrap-datetimepicker.min.css')}}">
@endif

@if (Route::is(['email-reply', 'file-manager', 'gallery', 'search-list', 'social-feed']))
    <!-- Fancybox CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/fancybox/jquery.fancybox.min.css')}}">
@endif
...
@if (Route::is(['file-manager']))
    <!-- Player CSS -->
    <link rel="stylesheet" href="{{ url('theme/css/plyr.css')}}">
@endif

@if (Route::is(['chart-c3']))
    <!-- ChartC3 CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/c3-chart/c3.min.css')}}">
@endif

@if (Route::is(['chart-morris']))
    <!-- Morris CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/morris/morris.css')}}">
@endif

@if (Route::is(['form-pickers', 'maps-leaflet', 'ui-clipboard', 'ui-drag-drop', 'ui-sortable', 'ui-swiperjs']))
    <!-- Dragula CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/dragula/css/dragula.min.css')}}">
@endif

@if (Route::is(['form-editors']))
    <!-- Quill css -->
    <link href="{{ url('theme/plugins/quill/quill.core.css')}}" rel="stylesheet" type="text/css" />
    <link href="{{ url('theme/plugins/quill/quill.snow.css')}}" rel="stylesheet" type="text/css" />
    <link href="{{ url('theme/plugins/quill/quill.bubble.css')}}" rel="stylesheet" type="text/css" />
@endif

@if (Route::is(['form-pickers', 'form-range-slider']))
    <!-- Vendor css -->
    <link href="{{ url('theme/css/vendor.min.css')}}" rel="stylesheet">
@endif

@if (Route::is(['form-range-slider']))
    <!-- nouisliderribute css -->
    <link rel="stylesheet" href="{{ url('theme/plugins/nouislider/nouislider.min.css')}}">
@endif

@if (Route::is(['form-wizard']))
    <!-- Wizard CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/twitter-bootstrap-wizard/form-wizard.css')}}">
@endif

@if (Route::is(['maps-leaflet']))
    <!-- Leaflet Maps CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/leaflet/leaflet.css')}}">
@endif

@if (Route::is(['maps-vector']))
    <!-- Jsvector Maps -->
    <link rel="stylesheet" href="{{ url('theme/plugins/jsvectormap/css/jsvectormap.min.css')}}">
@endif

@if (Route::is(['ui-lightbox']))
    <!-- Glightbox CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/lightbox/glightbox.min.css')}}">
@endif

@if (Route::is(['ui-sortable','ui-swiperjs']))
    <link rel="stylesheet" href="{{ url('theme/plugins/swiper/swiper-bundle.min.css')}}">
@endif

@if (Route::is(['ui-toasts']))
    <!-- Toatr CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/toastr/toatr.css')}}">
@endif
...
@if (Route::is(['account-statement', 'add-credit-notes', 'annual-report', 'balance-sheet', 'bank-accounts-settings', 'bank-accounts', 'best-seller', 'blogs', 'cash-flow', 'cities', 'countries', 'credit-notes', 'customer-details', 'customer-due-report', 'customer-invoice-report', 'customer-payment-summary', 'customer-recurring-invoices', 'customer-transactions', 'customers-report', 'customers', 'edit-credit-notes', 'expense-report', 'expenses', 'faq', 'income-report', 'incomes', 'inventory-report', 'low-stock', 'membership-addons', 'membership-transactions', 'money-transfer', 'payment-summary', 'payments', 'profit-loss-report', 'purchase-order-report', 'purchase-orders-report', 'purchase-return-report', 'purchases-report', 'recurring-invoices', 'sales-orders', 'sales-report', 'sales-returns', 'sold-stock', 'states', 'stock-history', 'stock-summary', 'supplier-payments', 'supplier-report', 'suppliers', 'tax-report', 'testimonials', 'transactions', 'trial-balance', 'ui-rangeslider', 'ui-rating']))
    <!-- Rangeslider CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/ion-rangeslider/css/ion.rangeSlider.css')}}">
    <link rel="stylesheet" href="{{ url('theme/plugins/ion-rangeslider/css/ion.rangeSlider.min.css')}}">
@endif

@if (Route::is(['ui-stickynote']))
    <!-- Sticky CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/stickynote/sticky.css')}}">
@endif

@if (Route::is(['form-select2']))
    <!-- Daterangepikcer CSS -->
    <link rel="stylesheet" href="{{ url('theme/plugins/choices.js/public/urls/styles/choices.min.css')}}">
@endif
