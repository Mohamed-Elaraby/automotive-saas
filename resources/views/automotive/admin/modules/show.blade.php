        ...
        <div class="content content-two">
            ...
            <div class="row">
                <div class="col-xl-8 d-flex">
                    <div class="card flex-fill">
                        <div class="card-header">
                            <h5 class="card-title mb-0">{{ __('tenant.module_overview') }}</h5>
                        </div>
                        <div class="card-body">
                            <div class="mb-3">
                                <div class="text-muted small mb-1">{{ __('tenant.focused_product') }}</div>
                                <h5 class="mb-1">{{ $focusedWorkspaceProduct['product_name'] ?? __('tenant.workspace_product') }}</h5>
                                <p class="mb-0 text-muted">
                                    {{ $focusedWorkspaceProduct['plan_name'] ?? __('tenant.no_plan_mapped') }} ·
                                    {{ !empty($focusedWorkspaceProduct['is_accessible']) ? __('tenant.connected') : ($focusedWorkspaceProduct['status_label'] ?? __('tenant.unavailable')) }}
                                </p>
                            </div>
                ...
                <div class="col-xl-4 d-flex">
                    <div class="card flex-fill">
                        <div class="card-header">
                            <h5 class="card-title mb-0">{{ __('tenant.quick_links') }}</h5>
                        </div>
                        <div class="card-body">
            ...
            @if(($page ?? '') === 'workshop-operations')
                <div class="row">
                    <div class="col-xl-3 col-md-6 d-flex"><div class="card flex-fill"><div class="card-body"><div class="text-muted small mb-1">{{ __('tenant.customers') }}</div><h4 class="mb-1">{{ ($moduleData['customers'] ?? collect())->count() }}</h4><p class="mb-0 text-muted">{{ __('tenant.workshop_customer_records_ready') }}</p></div></div></div>
                    <div class="col-xl-3 col-md-6 d-flex"><div class="card flex-fill"><div class="card-body"><div class="text-muted small mb-1">{{ __('tenant.vehicles') }}</div><h4 class="mb-1">{{ ($moduleData['vehicles'] ?? collect())->count() }}</h4><p class="mb-0 text-muted">{{ __('tenant.vehicles_linked_to_service_history') }}</p></div></div></div>
                    <div class="col-xl-3 col-md-6 d-flex"><div class="card flex-fill"><div class="card-body"><div class="text-muted small mb-1">{{ __('tenant.open_work_orders') }}</div><h4 class="mb-1">{{ ($moduleData['open_work_orders'] ?? collect())->count() }}</h4><p class="mb-0 text-muted">{{ __('tenant.jobs_currently_in_workshop_flow') }}</p></div></div></div>
                    <div class="col-xl-3 col-md-6 d-flex"><div class="card flex-fill"><div class="card-body"><div class="text-muted small mb-1">{{ __('tenant.accounting_handoffs') }}</div><h4 class="mb-1">{{ ($moduleData['recent_accounting_events'] ?? collect())->count() }}</h4><p class="mb-0 text-muted">{{ __('tenant.completed_jobs_posted_to_accounting') }}</p></div></div></div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body py-3">
                                <div class="d-flex flex-wrap gap-2">
                                    <a href="{{ route('automotive.admin.modules.workshop-customers', $workspaceQuery) }}" class="btn btn-outline-light">{{ __('tenant.customers_table') }}</a>
                                    <a href="{{ route('automotive.admin.modules.workshop-vehicles', $workspaceQuery) }}" class="btn btn-outline-light">{{ __('tenant.vehicles_table') }}</a>
                                    <a href="{{ route('automotive.admin.modules.workshop-work-orders', $workspaceQuery) }}" class="btn btn-outline-light">{{ __('tenant.work_orders_table') }}</a>
                                    <a href="{{ route('automotive.admin.modules.general-ledger', ['workspace_product' => 'accounting']) }}" class="btn btn-outline-light">{{ __('tenant.accounting_events') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-xl-4 d-flex">
                        <div class="card flex-fill"><div class="card-header"><h5 class="card-title mb-0">{{ __('tenant.step_create_customer') }}</h5></div><div class="card-body"><form method="POST" action="{{ route('automotive.admin.modules.workshop-operations.customers.store', $workspaceQuery) }}">@csrf<input type="hidden" name="workspace_product" value="{{ $workspaceQuery['workspace_product'] ?? data_get($focusedWorkspaceProduct, 'product_code', 'automotive_service') }}"><div class="mb-3"><label class="form-label">{{ __('tenant.customer_name') }}</label><input type="text" name="name" class="form-control" value="{{ old('name') }}"></div><div class="mb-3"><label class="form-label">{{ __('tenant.phone') }}</label><input type="text" name="phone" class="form-control" value="{{ old('phone') }}"></div><div class="mb-3"><label class="form-label">{{ __('tenant.email') }}</label><input type="email" name="email" class="form-control" value="{{ old('email') }}"></div><button type="submit" class="btn btn-outline-primary">{{ __('tenant.create_customer') }}</button></form></div></div>
                    </div>
                    <div class="col-xl-4 d-flex">
                        <div class="card flex-fill"><div class="card-header"><h5 class="card-title mb-0">{{ __('tenant.step_register_vehicle') }}</h5></div><div class="card-body">@if(($moduleData['customers'] ?? collect())->isEmpty())<p class="text-muted mb-0">{{ __('tenant.create_customer_first') }}</p>@else<form method="POST" action="{{ route('automotive.admin.modules.workshop-operations.vehicles.store', $workspaceQuery) }}">@csrf<input type="hidden" name="workspace_product" value="{{ $workspaceQuery['workspace_product'] ?? data_get($focusedWorkspaceProduct, 'product_code', 'automotive_service') }}"><div class="mb-3"><label class="form-label">{{ __('tenant.customer') }}</label><select name="customer_id" class="form-select">@foreach(($moduleData['customers'] ?? collect()) as $customer)<option value="{{ $customer->id }}">{{ $customer->name }}</option>@endforeach</select></div><div class="mb-3"><label class="form-label">{{ __('tenant.make') }}</label><input type="text" name="make" class="form-control" value="{{ old('make') }}"></div><div class="mb-3"><label class="form-label">{{ __('tenant.model') }}</label><input type="text" name="model" class="form-control" value="{{ old('model') }}"></div><div class="row"><div class="col-md-6 mb-3"><label class="form-label">{{ __('tenant.year') }}</label><input type="number" name="year" class="form-control" value="{{ old('year') }}"></div><div class="col-md-6 mb-3"><label class="form-label">{{ __('tenant.plate_number') }}</label><input type="text" name="plate_number" class="form-control" value="{{ old('plate_number') }}"></div></div><div class="mb-3"><label class="form-label">{{ __('tenant.vin') }}</label><input type="text" name="vin" class="form-control" value="{{ old('vin') }}"></div><button type="submit" class="btn btn-outline-primary">{{ __('tenant.create_vehicle') }}</button></form>@endif</div></div>
                    </div>
                    <div class="col-xl-4 d-flex">
                        <div class="card flex-fill"><div class="card-header"><h5 class="card-title mb-0">{{ __('tenant.step_create_work_order') }}</h5></div><div class="card-body"><form method="POST" action="{{ route('automotive.admin.modules.workshop-operations.work-orders.store', $workspaceQuery) }}">@csrf<input type="hidden" name="workspace_product" value="{{ $workspaceQuery['workspace_product'] ?? data_get($focusedWorkspaceProduct, 'product_code', 'automotive_service') }}"><div class="mb-3"><label class="form-label">{{ __('tenant.branch') }}</label><select name="branch_id" class="form-select">@foreach(($moduleData['active_branches'] ?? collect()) as $branch)<option value="{{ $branch->id }}" {{ (string) old('branch_id') === (string) $branch->id ? 'selected' : '' }}>{{ $branch->name }} ({{ $branch->code }})</option>@endforeach</select></div><div class="mb-3"><label class="form-label">{{ __('tenant.customer') }}</label><select name="customer_id" class="form-select"><option value="">{{ __('tenant.no_customer_linked') }}</option>@foreach(($moduleData['customers'] ?? collect()) as $customer)<option value="{{ $customer->id }}" {{ (string) old('customer_id') === (string) $customer->id ? 'selected' : '' }}>{{ $customer->name }}{{ $customer->phone ? ' · '.$customer->phone : '' }}</option>@endforeach</select></div><div class="mb-3"><label class="form-label">{{ __('tenant.vehicle') }}</label><select name="vehicle_id" class="form-select"><option value="">{{ __('tenant.no_vehicle_linked') }}</option>@foreach(($moduleData['vehicles'] ?? collect()) as $vehicle)<option value="{{ $vehicle->id }}" {{ (string) old('vehicle_id') === (string) $vehicle->id ? 'selected' : '' }}>{{ $vehicle->make }} {{ $vehicle->model }}{{ $vehicle->plate_number ? ' · '.$vehicle->plate_number : '' }}{{ $vehicle->customer ? ' · '.$vehicle->customer->name : '' }}</option>@endforeach</select></div><div class="mb-3"><label class="form-label">{{ __('tenant.work_order_title') }}</label><input type="text" name="title" class="form-control" value="{{ old('title') }}" placeholder="Brake service, engine inspection, etc."></div><div class="mb-3"><label class="form-label">{{ __('tenant.notes') }}</label><textarea name="notes" class="form-control" rows="3">{{ old('notes') }}</textarea></div><button type="submit" class="btn btn-primary">{{ __('tenant.create_work_order') }}</button></form></div></div>
                    </div>
                </div>

                <div class="row">
                    <div class="col-xl-5 d-flex">
                        <div class="card flex-fill"><div class="card-header"><h5 class="card-title mb-0">{{ __('tenant.step_consume_spare_parts') }}</h5></div><div class="card-body">@if(empty($moduleData['has_connected_parts_workspace']))<p class="text-muted mb-0">{{ __('tenant.connect_parts_first') }}</p>@elseif(($moduleData['available_stock_items'] ?? collect())->isEmpty())<p class="text-muted mb-0">{{ __('tenant.no_available_stock_items') }}</p>@elseif(($moduleData['open_work_orders'] ?? collect())->isEmpty())<p class="text-muted mb-0">{{ __('tenant.create_work_order_first') }}</p>@else<form method="POST" action="{{ route('automotive.admin.modules.workshop-operations.consume-part', $workspaceQuery) }}">@csrf<input type="hidden" name="workspace_product" value="{{ $workspaceQuery['workspace_product'] ?? data_get($focusedWorkspaceProduct, 'product_code', 'automotive_service') }}"><div class="mb-3"><label class="form-label">{{ __('tenant.work_order') }}</label><select name="work_order_id" class="form-select">@foreach(($moduleData['open_work_orders'] ?? collect()) as $workOrder)<option value="{{ $workOrder->id }}" {{ (string) old('work_order_id') === (string) $workOrder->id ? 'selected' : '' }}>{{ $workOrder->work_order_number }} - {{ $workOrder->title }} ({{ $workOrder->branch?->name }})</option>@endforeach</select></div><div class="mb-3"><label class="form-label">{{ __('tenant.stock_item') }}</label><select name="product_id" class="form-select">@foreach(($moduleData['available_stock_items'] ?? collect()) as $stockItem)<option value="{{ $stockItem->product_id }}" {{ (string) old('product_id') === (string) $stockItem->product_id ? 'selected' : '' }}>{{ $stockItem->product_name }} ({{ $stockItem->product_sku }}) - {{ rtrim(rtrim((string) $stockItem->quantity, '0'), '.') }} {{ $stockItem->product_unit }}</option>@endforeach</select></div><div class="mb-3"><label class="form-label">{{ __('tenant.branch') }}</label><select name="branch_id" class="form-select">@foreach(($moduleData['available_stock_items'] ?? collect())->unique('branch_id') as $stockItem)<option value="{{ $stockItem->branch_id }}" {{ (string) old('branch_id') === (string) $stockItem->branch_id ? 'selected' : '' }}>{{ $stockItem->branch_name }} ({{ $stockItem->branch_code }})</option>@endforeach</select></div><div class="mb-3"><label class="form-label">{{ __('tenant.quantity') }}</label><input type="number" step="0.001" min="0.001" name="quantity" class="form-control" value="{{ old('quantity', 1) }}"></div><div class="mb-3"><label class="form-label">{{ __('tenant.notes') }}</label><textarea name="notes" class="form-control" rows="3">{{ old('notes', __('tenant.workshop_consumption')) }}</textarea></div><button type="submit" class="btn btn-primary">{{ __('tenant.consume_stock') }}</button></form>@endif</div></div>
                    </div>
                    <div class="col-xl-7 d-flex">
                        <div class="card flex-fill"><div class="card-header"><h5 class="card-title mb-0">{{ __('tenant.recent_work_orders') }}</h5></div><div class="card-body">@forelse(($moduleData['recent_work_orders'] ?? collect()) as $workOrder)<div class="border-bottom pb-2 mb-2"><div class="d-flex justify-content-between align-items-start"><div><h6 class="mb-1">{{ $workOrder->work_order_number }}</h6><div class="text-muted small">{{ $workOrder->title }}</div><div class="text-muted small">{{ $workOrder->branch?->name ?? '—' }} · {{ $workOrder->creator?->name ?? __('tenant.system_user') }}</div>@if($workOrder->customer || $workOrder->vehicle)<div class="text-muted small">{{ $workOrder->customer?->name ?: __('tenant.no_customer_linked') }}@if($workOrder->vehicle) · {{ $workOrder->vehicle->make }} {{ $workOrder->vehicle->model }}{{ $workOrder->vehicle->plate_number ? ' · '.$workOrder->vehicle->plate_number : '' }}@endif</div>@endif</div><div class="text-end"><span class="badge {{ in_array($workOrder->status, ['open', 'in_progress'], true) ? 'bg-success' : 'bg-secondary' }}">{{ strtoupper(str_replace('_', ' ', $workOrder->status)) }}</span><div class="mt-2"><a href="{{ route('automotive.admin.modules.workshop-operations.work-orders.show', ['workOrder' => $workOrder->id] + $workspaceQuery) }}" class="btn btn-sm btn-outline-light">{{ __('tenant.open_record') }}</a></div></div></div></div>@empty<p class="text-muted mb-0">{{ __('tenant.no_work_orders_created') }}</p>@endforelse</div></div>
                    </div>
                </div>

                <div class="row">
                    <div class="col-xl-6 d-flex"><div class="card flex-fill"><div class="card-header"><h5 class="card-title mb-0">{{ __('tenant.available_spare_parts_stock') }}</h5></div><div class="card-body">@forelse(($moduleData['available_stock_items'] ?? collect()) as $stockItem)<div class="border-bottom pb-2 mb-2"><div class="d-flex justify-content-between align-items-start"><div><h6 class="mb-1">{{ $stockItem->product_name }}</h6><div class="text-muted small">{{ $stockItem->product_sku }} · {{ $stockItem->branch_name }} ({{ $stockItem->branch_code }})</div></div><span class="badge bg-success">{{ rtrim(rtrim((string) $stockItem->quantity, '0'), '.') }} {{ $stockItem->product_unit }}</span></div></div>@empty<p class="text-muted mb-0">{{ __('tenant.no_stock_snapshot') }}</p>@endforelse</div></div></div>
                    <div class="col-xl-6 d-flex"><div class="card flex-fill"><div class="card-header"><h5 class="card-title mb-0">{{ __('tenant.recent_accounting_handoffs') }}</h5></div><div class="card-body">@forelse(($moduleData['recent_accounting_events'] ?? collect()) as $event)<div class="border-bottom pb-2 mb-2"><div class="d-flex justify-content-between align-items-start"><div><h6 class="mb-1">{{ data_get($event->payload, 'work_order_number', __('tenant.work_order_fallback')) }}</h6><div class="text-muted small">{{ data_get($event->payload, 'title', __('tenant.completed_workshop_job')) }}</div><div class="text-muted small">{{ $event->creator_name ?: __('tenant.system_user') }}</div></div><div class="text-end"><span class="badge bg-info">{{ strtoupper($event->status) }}</span><div class="text-muted small mt-1">{{ number_format((float) $event->total_amount, 2) }} {{ $event->currency }}</div></div></div></div>@empty<p class="text-muted mb-0">{{ __('tenant.no_completed_work_orders_handed_off') }}</p>@endforelse</div></div></div>
                </div>

                <div class="row">
                    <div class="col-xl-12 d-flex"><div class="card flex-fill"><div class="card-header"><h5 class="card-title mb-0">{{ __('tenant.recent_workshop_consumptions') }}</h5></div><div class="card-body">@forelse(($moduleData['recent_workshop_consumptions'] ?? collect()) as $movement)<div class="border-bottom pb-2 mb-2"><div class="d-flex justify-content-between align-items-start"><div><h6 class="mb-1">{{ $movement->product_name }}</h6><div class="text-muted small">{{ $movement->product_sku }} · {{ $movement->branch_name }}</div><div class="text-muted small">{{ $movement->work_order_number ?: __('tenant.no_work_order') }}@if(!empty($movement->work_order_title)) · {{ $movement->work_order_title }}@endif</div><div class="text-muted small">{{ $movement->creator_name ?: __('tenant.system_user') }} · {{ optional($movement->movement_date)->format('Y-m-d H:i') }}</div></div><div class="text-end"><span class="badge bg-warning text-dark">{{ rtrim(rtrim((string) $movement->quantity, '0'), '.') }}</span><div class="text-muted small mt-1">{{ $movement->notes ?: __('tenant.workshop_consumption') }}</div></div></div></div>@empty<p class="text-muted mb-0">{{ __('tenant.no_workshop_stock_consumption') }}</p>@endforelse</div></div></div>
                </div>
            @elseif(($page ?? '') === 'workshop-customers')
                <div class="card"><div class="card-header"><h5 class="card-title mb-0">{{ __('tenant.customers_table') }}</h5></div><div class="card-body">@forelse(($moduleData['customers'] ?? collect()) as $customer)<div class="border-bottom pb-2 mb-2"><h6 class="mb-1">{{ $customer->name }}</h6><div class="text-muted small">{{ $customer->phone ?: __('tenant.no_phone') }}{{ $customer->email ? ' · '.$customer->email : '' }}</div></div>@empty<p class="text-muted mb-0">{{ __('tenant.no_workshop_customers') }}</p>@endforelse</div></div>
            @elseif(($page ?? '') === 'workshop-vehicles')
                <div class="card"><div class="card-header"><h5 class="card-title mb-0">{{ __('tenant.vehicles_table') }}</h5></div><div class="card-body">@forelse(($moduleData['vehicles'] ?? collect()) as $vehicle)<div class="border-bottom pb-2 mb-2"><h6 class="mb-1">{{ $vehicle->make }} {{ $vehicle->model }}</h6><div class="text-muted small">{{ $vehicle->plate_number ?: __('tenant.no_plate') }}{{ $vehicle->customer ? ' · '.$vehicle->customer->name : '' }}</div></div>@empty<p class="text-muted mb-0">{{ __('tenant.no_workshop_vehicles') }}</p>@endforelse</div></div>
            @elseif(($page ?? '') === 'workshop-work-orders')
                <div class="card"><div class="card-header"><h5 class="card-title mb-0">{{ __('tenant.work_orders_table') }}</h5></div><div class="card-body">@forelse(($moduleData['recent_work_orders'] ?? collect()) as $workOrder)<div class="border-bottom pb-2 mb-2"><div class="d-flex justify-content-between align-items-start"><div><h6 class="mb-1">{{ $workOrder->work_order_number }}</h6><div class="text-muted small">{{ $workOrder->title }}</div><div class="text-muted small">{{ $workOrder->customer?->name ?: __('tenant.no_customer_linked') }}{{ $workOrder->vehicle ? ' · '.$workOrder->vehicle->make.' '.$workOrder->vehicle->model : '' }}</div></div><div class="text-end"><span class="badge {{ in_array($workOrder->status, ['open', 'in_progress'], true) ? 'bg-success' : 'bg-secondary' }}">{{ strtoupper(str_replace('_', ' ', $workOrder->status)) }}</span><div class="mt-2"><a href="{{ route('automotive.admin.modules.workshop-operations.work-orders.show', ['workOrder' => $workOrder->id] + $workspaceQuery) }}" class="btn btn-sm btn-outline-light">{{ __('tenant.open_record') }}</a></div></div></div></div>@empty<p class="text-muted mb-0">{{ __('tenant.no_work_orders_created') }}</p>@endforelse</div></div>
