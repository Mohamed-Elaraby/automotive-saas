...
use App\Services\Billing\CouponEligibilityService;
...
public function show(Coupon $coupon): View
{
    ...
    $latestRedemptions = $coupon->redemptions()
        ->latest('id')
        ->take(20)
        ->get();
    ...
    return view('admin.coupons.show', [
        ...
        'previewInput' => session('previewInput', [
            'tenant_id' => '',
            'plan_id' => '',
            'is_first_billing_cycle' => '1',
        ]),
    ]);
}

public function previewEligibility(Request $request, Coupon $coupon): RedirectResponse
{
    $validated = $request->validate([
        'tenant_id' => ['nullable', 'string', 'max:255'],
        'plan_id' => ['nullable', 'integer'],
        'is_first_billing_cycle' => ['nullable', 'boolean'],
    ]);

    $result = $this->eligibilityService->evaluate(
        coupon: $coupon,
            tenantId: filled($validated['tenant_id'] ?? null) ? (string) $validated['tenant_id'] : null,
            planId: ! empty($validated['plan_id']) ? (int) $validated['plan_id'] : null,
            isFirstBillingCycle: (bool) ($validated['is_first_billing_cycle'] ?? false),
        );

        return redirect()
            ->route('admin.coupons.show', $coupon)
            ->with('previewResult', $result)
            ->with('previewInput', [
                'tenant_id' => $validated['tenant_id'] ?? '',
                'plan_id' => (string) ($validated['plan_id'] ?? ''),
                'is_first_billing_cycle' => (string) ((int) ((bool) ($validated['is_first_billing_cycle'] ?? false))),
            ]);
    }
